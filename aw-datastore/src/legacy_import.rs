...
mod import {
    ...
    fn get_legacy_buckets(conn: &Connection) -> Result<Vec<Bucket>, LegacyDatastoreImportError> {
        ...
        let bucket_rows = match stmt.query_map(&[] as &[&dyn ToSql], |row| {
            ...
            Ok(buckets) => buckets,
            Err(err) => return Err(LegacyDatastoreImportError::SQLMapError(format!(
    ...
    fn get_legacy_events(conn: &Connection, bucket_id: &i64) -> Result<Vec<Event>, LegacyDatastoreImportError> {
        let mut stmt = match conn.prepare("
            ...
            ;") {
            Ok(stmt) => stmt,
            Err(err) => return Err(LegacyDatastoreImportError::SQLPrepareError(
                err.to_string()))
        };
        ...
        let rows = match stmt.query_map(&[&bucket_id], |row| {
            ...
            let data : serde_json::map::Map<String, serde_json::Value> = match serde_json::from_str(&data_str) {
                Ok(data) => data,
                Err(err) => panic!("Unable to parse JSON data in event from bucket {}\n{}\n{}",
    ...
    pub fn legacy_import(new_ds: &mut DatastoreInstance, new_conn: &Connection) -> Result<(), LegacyDatastoreImportError> {
        ...
        for bucket in &buckets {
            ...
            match new_ds.insert_events(new_conn, &bucket.id, events) {
                Ok(_) => (),
                Err(err) => panic!("Failed to insert events to bucket '{}': {:?}", bucket.id, err)
            };
    ...
    fn test_legacy_import() {
        ...
        for (bucket_id, _bucket) in buckets {
            let events = ds.get_events(&new_conn, &bucket_id, None, None, Some(1000)).unwrap();
...
mod import {
    ...
    pub fn legacy_import(_new_ds: &mut DatastoreInstance, _new_conn: &Connection) -> Result<(), LegacyDatastoreImportError> {
        Ok(())
    }
}
