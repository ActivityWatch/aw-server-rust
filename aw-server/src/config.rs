...
pub fn get_config() -> AWConfig {
    ...
    if !config_path.is_file() {
        ...
        let default_config_str = toml::to_string(&default_config).expect("Failed to convert default config to string");
    ...
    let mut rfile = File::open(config_path.clone()).expect("Failed to open config file for reading");
    let mut content = String::new();
    rfile.read_to_string(&mut content).expect("Failed to read config as a string");
